...
PROBLEM_CODES = {
    ...
    "reb10-no-overlap": 1006699,
    "reb10-small-overlap": 1016699,
    "reb15-small-overlap": 1516699,
    "reb20-small-overlap": 2016699,
    "reb25-small-overlap": 2516699,
    "reb30-small-overlap": 3016699,
    "reb10-large-overlap": 1046699,
...
def run_algorithm(config: RunConfig, in_dir=None, show_output=False, save_statistics=True,
    ...
    set_ranges = False
    for i in range(10):
        if f"reb{i}" in config.problem.lower() or "reb-chain" in config.problem.lower():
            set_ranges = True
            config.lower_init_bound, config.upper_init_bound = INIT_RANGES["reb"]
    if not set_ranges:
    ...
    command += f"0 "
    command += f"{"0" if config.gen_elitist else "1"} "
    command += f"0 "
    ...
    succeeded = (statistics["best_objective"].iloc[-1] <= config.vtr and
                 statistics["evaluations"].iloc[-1] <= config.max_num_evaluations and
                 statistics["seconds"].iloc[-1] <= config.max_num_seconds)
    chol_fails = statistics["cholesky_fails"].iloc[-1]

    if show_output:
        print(f"Succeeded: {succeeded}")

    return RunResult(config, statistics, chol_fails, succeeded)
