...
def run_vkd_cma(problem_name: str, dimensionality: int, population_size: int, show_output: bool, max_evaluations: int):
    ...
    tcoption['maxeval'] = max_evaluations  # int(5e3 * N * esoption['lam'])
    tcoption['tolf'] = 1e-16
    ...
    while r < n_restart:
        ...
        while not satisfied:
            ...
            if show_output and itr % 20 == 1:
                print(itr, tot_eval + vkd.neval, vkd.arf.min(), vkd.sigma, vkd.k)
        ...
        if show_output:
            print(f"Restarting. (left with {maxeval - tot_eval} evaluations)")
...
def main():
    ...
    parser.add_argument('-e', '--maximum-evaluations', type=int, default=1e10)
    ...
    generations, evaluations, seconds, best_objective = run_vkd_cma(
        args.problem.lower(), args.dimensionality, args.population_size, args.show_output, args.maximum_evaluations
    )
