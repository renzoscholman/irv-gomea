...
def run_pop_sweep(base_dir: str, base_run_config: RunConfig, num_repeats_per_config: int,
                  ...
                  incremental=False, gen_elitist=True):
    config_counter = [0]
    history = []
    history_counter = [0]
    ...
    def test_population_size(population_size: int):
        ...
        for repeat in range(num_repeats_per_config):
            ...
            derived_config.incremental = incremental
            derived_config.gen_elitist = gen_elitist
        ...
        with Pool(num_cpus) as pool:
            res = pool.imap_unordered(bisection_worker, configs)
            results = list(res)
        ...
        min_num_evals = np.min([
            (r.statistics["evaluations"].iloc[-1] if r.succeeded else DEFAULT_MAX_NUM_EVALUATIONS)
            for r in results
        ])
        
        succ_perc = np.mean([
            (1 if r.succeeded else 0)
            for r in results
        ])
        ...
        if log_progress:
            print(
                ...
                f"[NUM-EVALS] {median_num_evaluations:8}  (min {min_num_evals:8})"
                f"[SUCC PERC] {succ_perc:2f} "
    ...
    for pop_size in range(160, 220, 10):
