...
int cov_matrix_copy = 0;
short selection_during_gom,
        alter_full_fos_gom_acceptance,
...
// Full covariance setting

// Settings for iRV-GOMEA with p_accept = 0.05 and dmd = 0.9 OLD
// double alpha_ams[3] = {-1.110672, 0.279548, 0.379646};
// double alpha_cov[3] = {-1.102799, 1.695587, 2.338390};

// Settings for iRV-GOMEA with p_accept = 0.05 and dmd = 0.9 
// double alpha_ams[3] = {-0.878993, 0.463104, 0.525238};
// double alpha_cov[3] = {-1.458781, 1.454894, 2.132889};

// Settings for iRV-GOMEA with p_accept = 0.0 and dmd = 0.95 (dmi = 1/0.9)
// double alpha_ams[3] = {-4.870764, 0.435446, 0.965379}; 
// double alpha_cov[3] = {-1.375980, 1.379696, 1.998973}; 

// Settings for iRV-GOMEA with p_accept = 0.05 and dmd = 0.95 (dmi = 1/0.9)
double alpha_ams[3] = {-2.946668, 0.467165, 0.870257};
double alpha_cov[3] = {-1.0, 1.319554, 1.944011};

// Settings for iRV-GOMEA with p_accept = 0.0 and dmd = 0.9
// double alpha_ams[3] = {-4.458852, 0.573003, 1.099545};
// double alpha_cov[3] = {-1.447943, 1.359901, 1.994236};

// BOA settings
// double alpha_ams[3] = {-0.52, 0.7, 0.65}; 
// double alpha_cov[3] = {-0.33, 1.5, 1.1}; 
...
int *
greedyScatteredSubsetSelection(double **points, int number_of_points, int number_of_dimensions, int number_to_select) {
    ...
    if (number_to_select > number_of_points) {
        ...
        printf("Error: greedyScatteredSubsetSelection asked to select %d solutions from set of size %d.",
               number_to_select, number_of_points);
        printf("\n\n");
    ...
    for (i = 0; i < number_of_points - number_selected_so_far; i++)
        nn_distances[i] = distanceEuclidean(points[indices_left[i]], points[result[number_selected_so_far - 1]],
    ...
    while (number_selected_so_far < number_to_select) {
        ...
        for (i = 0; i < number_of_points - number_selected_so_far; i++) {
            value = distanceEuclidean(points[indices_left[i]], points[result[number_selected_so_far - 1]],
                                      number_of_dimensions);
