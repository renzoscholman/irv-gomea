...
fos_t::fos_t(const std::map<int, std::set<int>> &variable_interaction_graph) {
    ...
    for(int ind = 0; ind < variable_interaction_graph.size(); ind++) {
        std::vector<int> clique;
        std::set<int> cond_candidates;

        clique.push_back(ind);

        for (int x: variable_interaction_graph.at(ind)) // neighbors of "seed" ind
        {
            bool add_to_clique = true;
            // Get neighbours of neighbour x
            ...
            if (add_to_clique) {
                for (int y: clique) {
                    // Do not add x to clique if it has a neighbour that is not a neighbour of "seed" ind
    ...
    conditional_distribution_t *full_cond = new conditional_distribution_t();
        ...
        if (!clique_is_not_unique) {
            if (include_cliques_as_fos_elements) {
                if (use_conditional_sampling) {
                    std::set<int> clique_cond;
                    std::vector<int> clique;
                    for(int i : clique_candidate_list[clique_a]){
                        bool valid_clique = true;
                        bool valid_cond = false;
                        for(auto distribution : this->distributions){
                            valid_clique = valid_clique && std::find(distribution->variables.begin(), distribution->variables.end(), i) == distribution->variables.end();
                            valid_cond = valid_cond || std::find(distribution->variables.begin(), distribution->variables.end(), i) != distribution->variables.end();
                        }
                        if(valid_clique && !valid_cond){
                            clique.push_back(i);
                        }
                        if(valid_cond){
                            clique_cond.insert(i);
                        }
                    }

                    addConditionedGroup(clique, clique_cond);
                    full_cond->addGroupOfVariables(clique, clique_cond);
                } else {
...
fos_t::fos_t(const std::map<int, std::set<int>> &variable_interaction_graph,
    ...
    if (seed_cliques_per_variable) {
        ...
        for (int i = 0; i < number_of_parameters; i++) {
            ...
            for (int x: variable_interaction_graph.at(ind)) // neighbors of "seed" ind
            {
                // Set neighbor x as conditional candidate
...
void fos_t::addGroup(std::vector<int> group) {
    ...
    sets_cond.push_back(std::vector<int>());
...
void fos_t::addConditionedGroup(std::vector<int> variables, std::set<int> conditioned_variables) {
    ...
    sets_cond.push_back(std::vector<int>(conditioned_variables.begin(), conditioned_variables.end()));
...
void fos_t::randomizeOrder() {
    ...
    // Set full fos element to first one
...
void fos_t::print() {
    if(use_conditional_sampling){
        for (distribution_t *d : distributions) {
            ...
            printf("|| \t(accepted %d out of %d) \n", d->gom_accepted, d->gom_total);
        }
        printf("\n\n");
    } else {
        printf("{");
...
void fos_t::applyPartialAMS(int FOS_index, partial_solution_t *solution, double delta_AMS) {
    distributions[FOS_index]->applyPartialAMS(solution, delta_AMS);
}

void fos_t::estimateDistributions(solution_t **selection, int selection_size,
                                  vec_t <vec_t<double>> fitness_dependency_matrix) {
    assert(false); // not updated to include num_gens parameter
    for (int i = 0; i < getLength(); i++)
...
void fos_t::copyInformation(fos_t *linkage_model) {
// Inherit distribution multipliers
    ...
    //     FOS_element_similarity_matrix[i] = (int *) Malloc((this->getLength()) * sizeof(int));
    // }
    ...
    //         int b = 0;
    //         int matches = 0;
    //         while (a < linkage_model->sets[i].size() && b < this->sets[j].size()) {
    //             if (linkage_model->sets[i][a] < this->sets[j][b]) {
    //                 a++;
    //             } else if (linkage_model->sets[i][a] > this->sets[j][b]) {
    //                 b++;
    //             } else {
    //                 a++;
    //                 b++;
    ...
    //             }
    //         }
    //         FOS_element_similarity_matrix[i][j] = (int) 10000 * (2.0 * matches / (linkage_model->sets[i].size() + this->sets[j].size()));
    //     }
    // }
    ...
    for (int i = 0; i < linkage_model->getLength(); i++) {
        for (int j = 0; j < this->getLength(); j++) {
            int size_i = linkage_model->sets[i].size();
            int size_j = this->sets[j].size();
            int a = 0;
            int b = 0;
            int matches = 0;
            while (a < size_i && b < size_j) {
                if (linkage_model->sets[i][a] < this->sets[j][b]) {
                    a++;
                } else if (linkage_model->sets[i][a] > this->sets[j][b]) {
                    b++;
            ...
            a = 0;
            b = 0;
            int size_cond_i = linkage_model->sets_cond[i].size();
            int size_cond_j = this->sets_cond[j].size();
            while (a < size_cond_i && b < size_cond_j) {
                if (linkage_model->sets_cond[i][a] < this->sets_cond[j][b]) {
                    a++;
                } else if (linkage_model->sets_cond[i][a] > this->sets_cond[j][b]) {
                    b++;
                } else {
                    a++;
                    b++;
            ...
            FOS_element_similarity_matrix[i][j] = (int) 10000 * (2.0 * matches / (size_i + size_j + size_cond_i + size_cond_j));
        }
    }
    ...
    //     printf("%02d: ", i);
    //     for (int j = 0; j < this->getLength(); j++) {
    //         printf("%d, ", FOS_element_similarity_matrix[i][j]);
    //     }
    //     printf("\n");
    // }
    // printf("\n");
    ...
    //     printf("%02d: ", j);
    //     int max_index = 0;
    //     for (int i = 1; i < linkage_model->getLength(); i++) {
    //         if(FOS_element_similarity_matrix[i][j] > FOS_element_similarity_matrix[max_index][j]) {
    //             max_index = i;
    //         }
    //     }
    //     printf(" FROM OLD ");
    //     for(int i : linkage_model->getSet(max_index)) {
    //         printf("%d, ", i);
    //     }
    //     printf(" TO NEW ");
    //     for(int i : this->getSet(j)) {
    //         printf("%d, ", i);
    //     }
    //     printf("\n");
    // }
    ...
    for (int i = 0; i < this->getLength(); i++) {
        ...
        std::set<int> to_find;
        to_find.insert(this->distributions[i]->variables.begin(), this->distributions[i]->variables.end());

        // printf("FOS element %d with vars ", i);
        // for (int param : this->distributions[i]->variables) {
        //     printf("%d, ", param);
        // }
        // printf(" | ");
        // for(int param : this->distributions[i]->conditionals){
        //     printf("%d, ", param);
        // }
        // printf("\t\t");

        // Copy distribution multipliers
        this->distributions[i]->distribution_multiplier = linkage_model->distributions[max_index]->distribution_multiplier;
        if(use_conditional_sampling){
            ((conditional_distribution_t *)this->distributions[i])->initializeMemory();
        }

        // In case of incremental learning of (co)variance, copy information from one or more distributions of previous linkage model
        if (use_incremental)
        {
            // We have a fos element in the previous linkage model that equals the current
            if (max_similarity == 10000){ 
                // printf("Copying from EQUAL index: %d with vars ", max_index);
                //     for(int param : linkage_model->distributions[max_index]->variables){
                //         printf("%d, ", param);
                //     }
                //     printf(" | ");
                //     for(int param : linkage_model->distributions[max_index]->conditionals){
                //         printf("%d, ", param);
                //     }
                //     printf("\n");
                if(use_conditional_sampling){
                    ((conditional_distribution_t *)this->distributions[i])->copyCovariances(((conditional_distribution_t *)linkage_model->distributions[max_index]));
                } else {
                    ((normal_distribution_t *)this->distributions[i])->copyCovariances(((normal_distribution_t *)linkage_model->distributions[max_index]));
                }
            } else {
                // Loop over all other fos elements to fill as much (co)variance information as possible
                while(to_find.size()){
                    bool found_any_fos = false;
                    for(int param : linkage_model->distributions[max_index]->variables){
                        to_find.erase(param);
                    }
                    // printf("Copying from index: %d with vars ", max_index);
                    // for(int param : linkage_model->distributions[max_index]->variables){
                    //     printf("%d, ", param);
                    // }
                    // printf(" | ");
                    ...
                    for (int j = 0; j < linkage_model->getLength(); j++) {
                        bool found_all = true;
                        for(int param : linkage_model->distributions[j]->variables){
                            if(to_find.find(param) == to_find.end()){
                                found_all = false;
                                break;
                            }
                        }
                    ...
                    if(!found_any_fos) break; 
                }

                // this->distributions[i]->choleskyDecomposition(((conditional_distribution_t *)this->distributions[i])->full_covariance_matrices[0]);
            }
        }
    }

    // if (use_incremental) 
    //     this->full_covariance = linkage_model->full_covariance;
