...
void population_t::estimateDistribution() {
    ...
    // if ((static_linkage_tree && use_conditional_sampling))
    // {
    ...
    // if(static_linkage_tree && use_conditional_sampling && use_incremental){
    //     initializeFOS(); // Does a reshuffling
    // }
...
// void population_t::estimateDistributionAMS(int FOS_index) {
//     linkage_model->estimateDistributionAMS(number_of_generations, FOS_index, selection, selection_size, mean_shift_vector);
// }
...
void population_t::updateAMSMeans() {
    double eta_ams = use_incremental ? 1.0 - exp(alpha_ams[0] * pow(selection_size, alpha_ams[1]) / pow(fitness->number_of_parameters, alpha_ams[2])) : 1.0;
    // double eta_ams = 1.0;
    // std::cout << eta_ams << std::endl;
    for (int i = 0; i < fitness->number_of_parameters; i++) {
        ...
        if (number_of_generations > 0){
            if(number_of_generations == 1){
                ...
                mean_shift_vector[i] = (1.0 - eta_ams) * mean_shift_vector[i] + eta_ams * (new_mean - prev_mean_vector[i]);
            }
        } else {
            mean_shift_vector[i] = 0.0;
        }

        prev_mean_vector[i] = new_mean;
    }
    // std::cout << this->number_of_generations << "\t\t";
    // for (int i = 0; i < fitness->number_of_parameters; i++)
    //     std::cout << mean_shift_vector[i] << ", ";
    // std::cout << std::endl;
}
...
void population_t::generateAndEvaluateNewSolutions() {
    ...
    if (perform_factorized_gom) {
        ...
        for (int g = 0; g < linkage_model->getLength(); g++) {
            ...
            double t = getTimer();
            double acceptance_rate = linkage_model->getAcceptanceRate();
            if(g == 0 && linkage_model->distributions[FOS_index]->variables.size() == fitness->number_of_parameters && alter_full_fos_gom_acceptance){
                acceptance_rate = 1.0;
            }
            ...
            for (int k = num_elitists_to_copy; k < population_size; k++) {
                ...
                linkage_model->distributions[FOS_index]->gom_total++;
            }

            if (number_of_generations > 0) {
                for (int k = num_elitists_to_copy; k <= number_of_AMS_solutions; k++)
                    applyPartialAMS(sampled_solutions[FOS_index][k], FOS_index);
            }
            ...
            for (int k = num_elitists_to_copy; k < population_size; k++) {
                ...
                if (accept_improvement[k]){
                    linkage_model->distributions[FOS_index]->gom_accepted++;
                    num_improvements++;
                }
            }

            for (int k = num_elitists_to_copy; k < population_size; k++) {
                if (accept_improvement[k] || randomRealUniform01() < acceptance_rate) {
...
void population_t::applyPartialAMS(partial_solution_t *solution, int FOS_index) {
    // this->linkage_model->applyPartialAMS(FOS_index, solution, delta_AMS);
    // return;
    double cmul = linkage_model->getDistributionMultiplier(FOS_index);
    ...
    while ((out_of_range == 1) && (shrink_factor > 1e-10)) {
        ...
        for (int m = 0; m < solution->num_touched_variables; m++) {
            ...
            // double ams_var = use_incremental ? linkage_model->distributions[FOS_index]->mean_shift_vectors[m] : mean_shift_vector[im];
...
void population_t::initializeFOS() {
    ...
    if (FOS_element_size > 0) {
        ...
        FOS_element_ub = 100;
        max_clique_size = FOS_element_ub;
        include_cliques_as_fos_elements = 1;
        include_full_fos_element = 1;
        seed_cliques_per_variable = 1;
        use_conditional_sampling = 1;
        fitness->initializeVariableInteractionGraph();
        // for(int i = 0; i < fitness->number_of_parameters; i++){
        //     std::set<int> vig_i = fitness->variable_interaction_graph[i];
        //     printf("%d:\t\t", i);
        //     for(auto j : vig_i)
        //         printf("%d, ", j);
        //     printf("\n");
    ...
    else if (FOS_element_size == -4)
    {
        ...
        // printf("use_set_cover: %d, id: %d\n", use_set_cover, id);
        use_conditional_sampling = (id % 10) > 0;
        id /= 10;

        // printf("use_conditional_sampling: %d, id: %d\n", use_conditional_sampling, id);
        seed_cliques_per_variable = id % 10;
        id /= 10;
        // printf("seed_cliques_per_variable: %d, id: %d\n", seed_cliques_per_variable, id);
        ...
        // printf("is_fitness_based: %d, id: %d\n", is_fitness_based, id);
        if (is_fitness_based) {
            similarity_measure = 'F';
        ...
        // printf("include_full_fos_element: %d, id: %d\n", include_full_fos_element, id);
        include_cliques_as_fos_elements = (id % 10) == 1;
        id /= 10;

        // printf("include_cliques_as_fos_elements: %d, id: %d\n", include_cliques_as_fos_elements, id);
        max_clique_size = id;
    ...
    if (linkage_model != NULL) {
        new_FOS->copyInformation(linkage_model);
