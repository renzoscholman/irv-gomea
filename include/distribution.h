...
public:
    ...
    double distribution_multiplier_increase = 1.0 / 0.9;

    // Variables
    double distribution_multiplier = 1.0;
    double st_dev_ratio = 0.0;
    int samples_drawn = 0;
    ...
    virtual void estimateDistributionAMS(int selection_size, double * mean_shift_vector) = 0;

    virtual partial_solution_t *generatePartialSolution(solution_t *parent) = 0;

    virtual void applyPartialAMS(partial_solution_t *parent, double delta_AMS) = 0;
...
public:
    ...
    vec mean_shift_vector;
    bool copied_covariance_matrix;
    ...
    void estimateDistributionAMS(int selection_size, double * mean_shift_vector);

    void applyPartialAMS(partial_solution_t *parent, double delta_AMS);

    void copyCovariances(const normal_distribution_t * dist);
...
public:
    ...
    std::vector <std::vector<bool>> variables_copied;
    ...
    std::vector <vec> mean_shift_vectors;
    std::vector <vec> mean_vectors_conditioned_on;
    std::vector <mat> covariance_matrices_multiplied;
    std::vector<bool> copied_covariance_matrix;
    std::vector <mat> full_covariance_matrices;
    ...
    void copyCovariances(const conditional_distribution_t * dist);

    partial_solution_t *generatePartialSolution(solution_t *solution_conditioned_on = NULL);

    mat estimateRegularCovarianceMatrixML(int group_idx, std::vector<int> variables, vec mean_vector, solution_t **selection,
                                          int selection_size);

    mat estimateCovarianceMatrixML(int group_idx, std::vector<int> variables, solution_t **selection, int selection_size);
...
private:
    ...
    void estimateConditionalGaussianML(int variable_group_index, solution_t **selection, int selection_size,
                                       vec_t<vec_t<double>> fitness_dependency_matrix);
    void estimateConditionalAMSML(int variable_group_index, int selection_size, double * mean_shift_vector);
